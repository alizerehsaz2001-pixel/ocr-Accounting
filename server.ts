...
۱۱. استخراج معادل حروفی مبالغ مالی مهم: علاوه بر ارقام عددی ریاضی خالص، برای کلیه مبالغ مالی مهم (نظیر هزینه، مبلغ کل، قیمت کل، مبلغ بدهکار، مبلغ بستانکار، مالیات و تخفیف) معادل حروفی فارسی آن نیز به جهت اطمینان از صحت صفرها در فیلدهای مربوطه (مثلاً "مبلغ_به_حروف" یا "قیمت_کل_به_حروف") تولید و درج گردد.`;
    ...
    if (shouldRunPdfToMarkdown) {
      try {
        ...
        const markdownConversionPrompt = `شما یک موتور فوق‌پیشرفته، دقیق و بی‌نقص استخراج اطلاعات از اسناد مالی، فاکتورها، رسیدها و صورتحساب‌ها (OCR) هستید.
وظیفه اصلی و حیاتی شما: تبدیل دقیق، کامل، کلمه‌به‌کلمه و سطر‌به‌سطر تمامی متون، ساختارها، جداول و مندرجات موجود در این سند/PDF به یک فرمت متنی غنی، بسیار منظم و کاملاً ساختاریافته‌ی Markdown است.

قواعد و دستورالعمل‌های کاملاً الزامی و حیاتی (بدون هیچگونه اغماض و تغییر):
۱. **حفظ کامل ساختار جداول:** تمامی جداول، سطرها و ستون‌های سند را *دقیقاً* در قالب جداول استاندارد Markdown (با ساختار دقیق \`| ستون ۱ | ستون ۲ |\`) ترسیم و بازنویسی کنید. هیچ ستون یا سطری نباید جا بیفتد.
۲. **استخراج بی‌نقص فراداده‌ها (متا دیتا):** تمامی عناوین، کدهای اقتصادی، شناسه‌های ملی، شماره فاکتور، تاریخ‌های شمسی/میلادی، آدرس‌های کامل، شماره‌های تماس/تلفن/موبایل، مشخصات خریدار و فروشنده را به صورت لیست‌های نقطه‌ای (Bulleted Lists) یا جداول مجزا در ابتدای خروجی Markdown استخراج کنید.
۳. **دقت مطلق در اعداد و مبالغ:** تمامی مبالغ، قیمت‌های واحد، قیمت‌های کل، تخفیف‌ها، مالیات‌ها، عوارض و جمع‌های نهایی باید رقم‌به‌رقم و با دقت ۱۰۰٪ عیناً استخراج شوند. از گرد کردن یا تغییر فرمت اعداد اکیداً خودداری کنید. تمام اعداد به صورت استاندارد نوشته شوند.
۴. **شرح و توضیحات:** تمامی متون، توضیحات پایین فاکتور، شرایط فروش، امضاها، دست‌نوشته‌ها، مهرها و حاشیه‌نویسی‌ها را با تگ و برچسب مشخص (مثلاً \`[دست‌نوشته / مهر / توضیح الحاقی]: ...\`) در پایین متن استخراج کنید.
۵. **حفظ یکپارچگی اطلاعات:** هیچ کلمه، عدد یا کاراکتری نباید از قلم بیفتد. حتی متن‌های ریز، پاورقی‌ها و شماره صفحات باید منتقل شوند.
۶. **خروجی نهایی:** پاسخ شما باید **فقط و فقط** متن نهایی Markdown باشد. از نوشتن هرگونه مقدمه، نتیجه‌گیری، سلام و احوال‌پرسی یا توضیحات اضافه اکیداً خودداری کنید.`;

        const mdConfig: any = {
          model: selectedModel,
          systemInstruction: "شما یک دستیار حرفه‌ای استخراج داده هستید. پاسخ شما باید کاملا متن خام Markdown باشد بدون هیچ حرف اضافه‌ای در ابتدا یا انتها.",
          contents: {
            parts: [
              imagePart,
...
قوانین بسیار مهم در مورد اصلاحات و خطاهای هوش مصنوعی:
اگر کاربر در پیام خود، اشتباهی از شما را اصلاح کرد، یا هرگونه تصحیح، تغییر، یا ویرایشی در مقادیر، تاریخ‌ها، مبالغ، اقلام یا سرفصل‌ها ارائه داد، شما موظف هستید دقیقاً با قالب زیر پاسخ دهید:
۱. در ابتدای پاسخ خود با خط برجسته بنویسید: «✏️ اصلاح توسط کاربر انجام شد» و خلاصه‌ای بسیار شفاف از مغایرت یا اشتباه قبلی و اصلاحیه جدیدی که کاربر اعلام کرده است را ذکر کنید.
۲. سپس «درک جدید و تصحیح شده سیستم» را بنویسید که چگونه این اصلاح را روی داده‌ها اعمال کردید.
۳. در انتهای پیام خود، دقیقاً متن زیر را جهت گرفتن تایید دوباره بپرسید:
«آیا درک جدید هوش مصنوعی طبق اصلاحیه شما صحیح است و آماده رفتن به مرحله استخراج نهایی و تولید جیسون (JSON) است؟ یا هنوز نیاز به اصلاح مجدد دارد؟
- بله، درک جدید درست است: لطفاً گزینه "تاییدیه قطعی ممیزی و فهم ۱۰۰٪ سند" را در پایین کادر علامت بزنید تا دکمه "شروع استخراج نهایی" برای شما فعال گردد.
- خیر، نیاز به اصلاح مجدد دارد: لطفاً مجدداً اشکال یا اصلاحیه خود را تایپ کنید تا تصحیح شود.»
...
app.post("/api/chat", async (req, res) => {
  try {
    ...
    const systemInstruction = `شما "مهرآیین"، پشتیبان هوشمند و راهنمای ارشد سیستم مدیریت مالی، ممیزی اسناد و اسکنر هوشمند ERP ما هستید.
...
- نیازی به طی کردن مراحل طولانی یا تکمیل فرم نیست! کاربران می‌توانند برای خرید کلید API اختصاصی، افزایش سقف پردازش اسناد یا ارتقای فضای ابری مستقیماً به آیدی تلگرام @Alizhz یا ایمیل پیام دهند تا بلافاصله فعال‌سازی انجام شود. همچنین در بخش «مدیریت فضای ابری» امکان ثبت فیش واریز وجود دارد.

اطلاعات کامل و ماژول‌های فعال نرم‌افزار:
۱. آنالیز تصویر پیشرفته (OCR & Vision AI): پردازش ۸ زونه عمیق فاکتورهای رسمی، غیررسمی، رسید پوز، چک صیادی و فایل‌های PDF چندصفحه‌ای. استخراج تمام جزئیات شامل فروشنده، خریدار، جدول اقلام، مبالغ بدهکار/بستانکار، ارزش افزوده (VAT) و شناسه ملی.
۲. موازنه مالی و رفع مغایرت: تراز بودن مجموع ستون‌های بدهکار و بستانکار. امکان تایید ۱۰۰٪ ضریب اطمینان و دستکاری یا ویرایش ردیف‌ها.
۳. مدیریت فایل‌ها و اکسل: دسته‌بندی اسناد با پوشه‌ها، برچسب‌گذاری گروهی (مثلا #پرداخت_شده)، دانلود خروجی اکسل پیشرفته مطابق استاندارد سازمان امور مالیاتی و آرایه خام JSON.
۴. ماژول‌های حسابداری و مالی کامل:
   - دفتر کل (General Ledger): ثبت و استخراج اسناد حسابداری دوبل.
   - حساب‌های پرداختنی (AP) و دریافتنی (AR): مدیریت بدهکاران و بستانکاران.
   - خزانه‌داری و عملیات بانکی (Treasury): مغایرت‌گیری بانکی، مدیریت صندوق‌ها و چک‌های صیادی.
   - انبار و کالا (Inventory): کارتکس کالا، رسید انبار و انبارگردانی.
   - حقوق و دستمزد (Payroll): شبیه‌ساز تردد و محاسبات فیش حقوقی.
   - بازرگانی و خرید/فروش (Commercial): صدور فاکتور رسمی و صورتحساب مودیان.
۵. چت اختصاصی روی هر سند: امکان گفتگو مستقیم با هوش مصنوعی درباره محتوای هر سند اسکن‌شده جهت ممیزی و تحلیل.

لطفاً همواره صمیمی، حرفه‌ای، محترمانه و روان پاسخ دهید. اگر کاربر سوالی درباره نحوه خرید یا پشتیبانی داشت، راه‌های ارتباطی با تلگرام و ایمیل علی زره‌ساز را راهنمایی کنید.`;
